...
class ComparisonResultsProcessor: 
    ...
    def get_summary(self):
        ...
        return (self.__format_data__(similarities),
                self.__format_data__(program_1_overlap),
                self.__format_data__(program_2_overlap))
    ...
    def __proces_comparison_results__(self, raw_comparison_results):
        ...
        for comparison_result in raw_comparison_results:
            matches_a, matches_b, pair, raw_comparison_result = (comparison_result.matches_a, comparison_result.matches_b, comparison_result.pair, comparison_result.result)
            a_to_b_data = {}
        ...
        for compared_to_name, compared_to in program_to_program.items():
            for compared_program_name, compared_program in compared_to.items():
                ...
                compared_program["overlap_1"] = overlap_1 / np.sum([len(ast.ast) for ast in programs[compared_to_name].code_units]) if self.__config.assign_functions_based_on_types else number_of_comparisons
